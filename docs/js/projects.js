...
function openProject(index) {
  console.log("openProject called with index:", index);
  ...
  document.querySelector(".project-icon-large").textContent = data.icon;
  ...
  data.team.forEach((member) => {
    ...
    // Derive a friendly display name from the CV filename when name is missing or unreliable
    const deriveNameFromCv = (cvPath) => {
      if (!cvPath) return "";
      // Extract filename
      const parts = cvPath.split("/").pop().split("\\").pop();
      let name = parts.replace(/\.[^.]+$/, ""); // remove extension
      // Remove common suffixes (resume, cv, dates, and trailing authors)
      name = name.replace(/[_]+/g, " ");
      name = name.replace(/Resume.*$/i, "");
      name = name.replace(/CV.*$/i, "");
      name = name.replace(/ - .*$/g, "");
      name = name.replace(/\(.*\)$/g, "");
      return name.trim();
    };

    // Prefer explicit name unless it's a generic placeholder like "Team Member"
    const isGenericName =
      member.name && /team\s*member|member\s*\d+/i.test(member.name);
    const displayName =
      member.name && member.name.trim() && !isGenericName
        ? member.name
        : deriveNameFromCv(member.cv);

    memberCard.innerHTML = `
            <div class="member-avatar">${member.avatar || "👤"}</div>
            <h4 class="member-name">${displayName}</h4>
            ${member.cv ? '<p class="cv-hint">📄 Click to view CV</p>' : ""}
        `;
    ...
    if (member.cv) {
      ...
      memberCard.addEventListener("click", (e) => {
        e.stopPropagation();
        showMemberCV(displayName, member.cv);
      });
      // Add an explicit View CV button for clearer UX
      const btn = document.createElement("button");
      btn.className = "view-cv-btn";
      btn.textContent = "View CV";
      btn.style.cssText =
        "margin-top:8px;padding:6px 10px;border:none;border-radius:8px;background:linear-gradient(90deg,#6366f1,#ec4899);color:#fff;cursor:pointer;font-weight:600;";
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        showMemberCV(displayName, member.cv);
      });
      memberCard.appendChild(btn);
    }
...
function openProjectPdf(index) {
  ...
  const pdfPath = project.projectPdf;
  ...
  container.innerHTML = `<iframe src="${pdfPath}#toolbar=0" type="application/pdf"></iframe>`;
